...
func ErrEntityCanNotFound(entity string, err error) *AppError {
	return NewCustomError(
		err,
		fmt.Sprintf("User can not found %s", strings.ToLower(entity)),
		fmt.Sprintf("ErrUserCannotFound%s", entity),
	)
}
...
//var (const RecordNotFound = errors.New("Record not found"))
